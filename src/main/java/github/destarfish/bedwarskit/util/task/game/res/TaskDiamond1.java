...
public class TaskDiamond1 {
    ...
    public static void executeTask(Game game) {
        ...
        resourceSpawners.stream()
                ...
                .forEach(spawner -> spawner.getResources().stream()
                        ...
                        .findFirst()
                        .ifPresent(item -> spawner.setInterval(gametask_spawntime_diamond_1)));
    }

    public static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                .filter(Objects::nonNull)
                .filter(Player::isOnline)
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_diamond1_chat, "")) {
                        sendMessage(player, gametask_mess_diamond1_chat);
                    }
                    if (!Objects.equals(gametask_mess_diamond1_title, "")) {
                        String titleReal = gametask_mess_diamond1_title;
                        if (!Objects.equals(gametask_mess_diamond1_subtitle, "")) {
                            String subtitleReal = gametask_mess_diamond1_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_diamond1_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_diamond1_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_diamond1_actionbar, "")) {
                        sendActionBar(player, gametask_mess_diamond1_actionbar);
                    }
                });
    }
}
