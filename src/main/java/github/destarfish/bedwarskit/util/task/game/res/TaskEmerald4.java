...
public class TaskEmerald4 {
    ...
    public static void runTask(Game game) {
        ...
        int gameTaskTime = gametask_spawntime_tasks_emerald4;
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_emerald4_chat, "")) {
                        sendMessage(player, gametask_mess_emerald4_chat);
                    }
                    if (!Objects.equals(gametask_mess_emerald4_title, "")) {
                        String titleReal = gametask_mess_emerald4_title;
                        if (!Objects.equals(gametask_mess_emerald4_subtitle, "")) {
                            String subtitleReal = gametask_mess_emerald4_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_emerald4_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_emerald4_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_emerald4_actionbar, "")) {
                        sendActionBar(player, gametask_mess_emerald4_actionbar);
                    }
                });
    }
}
