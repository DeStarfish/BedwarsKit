...
public class TaskHealthSet1 {
    ...
    public static void runTask(Game game) {
        ...
        int gameTaskTime = gametask_time_healthset1;
    ...
    private static void executeTask(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    player.setHealthScale(gametask_healthset_1);
                });
    }
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .filter(Player::isOnline)
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_healthset1_chat, "")) {
                        sendMessage(player, gametask_mess_healthset1_chat);
                    }
                    if (!Objects.equals(gametask_mess_healthset1_title, "")) {
                        String titleReal = gametask_mess_healthset1_title;
                        if (!Objects.equals(gametask_mess_healthset1_subtitle, "")) {
                            String subtitleReal = gametask_mess_healthset1_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_healthset1_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_healthset1_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_healthset1_actionbar, "")) {
                        sendActionBar(player, gametask_mess_healthset1_actionbar);
                    }
                });
    }
}
