...
public class TaskGold1 {
    ...
    public static void runTask(Game game) {
        ...
        int gameTaskTime = gametask_spawntime_tasks_gold1;
    ...
    private static void executeTask(Game game) {
        ...
        resourceSpawners.stream()
                ...
                .forEach(spawner -> spawner.getResources().stream()
                        ...
                        .findFirst()
                        .ifPresent(item -> spawner.setInterval(gametask_spawntime_gold_1)));
    }
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_gold1_chat, "")) {
                        sendMessage(player, gametask_mess_gold1_chat);
                    }
                    if (!Objects.equals(gametask_mess_gold1_title, "")) {
                        String titleReal = gametask_mess_gold1_title;
                        if (!Objects.equals(gametask_mess_gold1_subtitle, "")) {
                            String subtitleReal = gametask_mess_gold1_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_gold1_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_gold1_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_gold1_actionbar, "")) {
                        sendActionBar(player, gametask_mess_gold1_actionbar);
                    }
                });
    }
}
