...
public class TaskEmerald1 {
    ...
    public static void runTask(Game game) {
        ...
        int gameTaskTime = gametask_spawntime_tasks_emerald1;
    ...
    private static void executeTask(Game game) {
        ...
        resourceSpawners.stream()
                ...
                .forEach(spawner -> spawner.getResources().stream()
                        ...
                        .findFirst()
                        .ifPresent(item -> spawner.setInterval(gametask_spawntime_emerald_1)));
    }
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_emerald1_chat, "")) {
                        sendMessage(player, gametask_mess_emerald1_chat);
                    }
                    if (!Objects.equals(gametask_mess_emerald1_title, "")) {
                        String titleReal = gametask_mess_emerald1_title;
                        if (!Objects.equals(gametask_mess_emerald1_subtitle, "")) {
                            String subtitleReal = gametask_mess_emerald1_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_emerald1_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_emerald1_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_emerald1_actionbar, "")) {
                        sendActionBar(player, gametask_mess_emerald1_actionbar);
                    }
                });
    }
}
