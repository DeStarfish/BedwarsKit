...
public class TaskHealthSet2 {
    ...
    public static void runTask(Game game) {
        ...
        int gameTaskTime = gametask_time_healthset2;
    ...
    private static void executeTask(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    player.setHealthScale(gametask_healthset_2);
                });
    }
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .filter(Player::isOnline)
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_healthset2_chat, "")) {
                        sendMessage(player, gametask_mess_healthset2_chat);
                    }
                    if (!Objects.equals(gametask_mess_healthset2_title, "")) {
                        String titleReal = gametask_mess_healthset2_title;
                        if (!Objects.equals(gametask_mess_healthset2_subtitle, "")) {
                            String subtitleReal = gametask_mess_healthset2_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_healthset2_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_healthset2_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_healthset2_actionbar, "")) {
                        sendActionBar(player, gametask_mess_healthset2_actionbar);
                    }
                });
    }
}
