...
public class TaskIron2 {
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_iron2_chat, "")) {
                        sendMessage(player, gametask_mess_iron2_chat);
                    }
                    if (!Objects.equals(gametask_mess_iron2_title, "")) {
                        String titleReal = gametask_mess_iron2_title;
                        if (!Objects.equals(gametask_mess_iron2_subtitle, "")) {
                            String subtitleReal = gametask_mess_iron2_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_iron2_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_iron2_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_iron2_actionbar, "")) {
                        sendActionBar(player, gametask_mess_iron2_actionbar);
                    }
                });
    }
}
