...
public class TaskDiamond2 {
    ...
    public static void runTask(Game game) {
        ...
        int gameTaskTime = gametask_spawntime_tasks_diamond2;
    ...
    private static void notifyPlayers(Game game) {
        game.getPlayers().stream()
                ...
                .forEach(player -> {
                    if (!Objects.equals(gametask_mess_diamond2_chat, "")) {
                        sendMessage(player, gametask_mess_diamond2_chat);
                    }
                    if (!Objects.equals(gametask_mess_diamond2_title, "")) {
                        String titleReal = gametask_mess_diamond2_title;
                        if (!Objects.equals(gametask_mess_diamond2_subtitle, "")) {
                            String subtitleReal = gametask_mess_diamond2_subtitle;

                            sendTitle(player, titleReal, subtitleReal);
                        }
                    } else if (!Objects.equals(gametask_mess_diamond2_subtitle, "")) {
                        String titleReal = " ";
                        String subtitleReal = gametask_mess_diamond2_subtitle;

                        sendTitle(player, titleReal, subtitleReal);
                    }
                    if (!Objects.equals(gametask_mess_diamond2_actionbar, "")) {
                        sendActionBar(player, gametask_mess_diamond2_actionbar);
                    }
                });
    }
}
